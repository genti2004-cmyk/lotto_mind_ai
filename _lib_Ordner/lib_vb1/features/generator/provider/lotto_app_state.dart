...
class LottoAppState extends ChangeNotifier {
  ...
  List<LottoTip> get savedTipsForSelectedDraw {
    final draw = _selectedDrawForCheck;
    if (draw == null) return const <LottoTip>[];
    return _savedTips
        .where((tip) => _tipCanBeEvaluatedAgainstDraw(tip, draw))
  ...
  Future<void> generateSystemTip() async {
    ...
    if (_systemPlayType == SystemPlayType.vew && _selectedSystemSize > 13) {
      throw Exception('Intervall max. 13 Zahlen erlaubt');
    }
  ...
  Future<int> saveSystemRowsAsTips() async {
    ...
    final targetDrawType = _targetDrawTypeForNewTip();
    final targetDrawDate = _nextDateForDrawType(targetDrawType);
      ...
      _savedTips.add(
        LottoTip(
          ...
          '${_systemPlayType == SystemPlayType.full
              ? 'voll'
              : 'intervall'}_$_selectedSystemSize',
          targetDrawType: targetDrawType,
          targetDrawDate: targetDrawDate,
          strategyName: _systemPlayType == SystemPlayType.full
              ? 'Vollsystem'
              : 'Intervall-System',
          strategyVersion: 2,
        ),
  ...
  Future<void> saveLastTip({String source = 'analysis'}) async {
    ...
    _savedTips.insert(
      ...
      LottoTip(
        ...
        targetDrawType: targetDrawType,
        targetDrawDate: targetDrawDate,
        strategyName: _strategyLabelFromSource(source),
        strategyVersion: 2,
      ),
    );
  ...
  void _rebuildTipTrackingEntries({bool notify = true}) {
    ...
    for (final tip in _savedTips.where((tip) => _tipCanBeEvaluatedAgainstDraw(tip, draw))) {
  ...
  DrawType _drawTypeForDate(DateTime date) {
    return drawTypeFromDate(date);
  }

  DrawType _targetDrawTypeForNewTip() {
    switch (_drawMode) {
      case DrawMode.wednesday:
  ...
  DrawType _nextScheduledDrawType(DateTime from) {
    final nextWednesday = _nextDateForWeekday(from, DateTime.wednesday);
    final nextSaturday = _nextDateForWeekday(from, DateTime.saturday);
    return nextWednesday.isBefore(nextSaturday)
        ? DrawType.wednesday
        : DrawType.saturday;
  }

  DateTime _nextDateForDrawType(DrawType type, {DateTime? from}) {
    final base = from ?? DateTime.now();
    switch (type) {
      case DrawType.wednesday:
        return _nextDateForWeekday(base, DateTime.wednesday);
      case DrawType.saturday:
        return _nextDateForWeekday(base, DateTime.saturday);
      case DrawType.unknown:
        return _nextDateForWeekday(base, DateTime.wednesday).isBefore(
          _nextDateForWeekday(base, DateTime.saturday),
        )
            ? _nextDateForWeekday(base, DateTime.wednesday)
            : _nextDateForWeekday(base, DateTime.saturday);
    }
  }
  ...
  bool _tipCanBeEvaluatedAgainstDraw(LottoTip tip, DrawResult draw) {
    final drawType = draw.drawType == DrawType.unknown
        ? _drawTypeForDate(draw.drawDate)
        : draw.drawType;

    if (tip.targetDrawType != DrawType.unknown &&
        tip.targetDrawType != drawType) {
      return false;
    }

    final targetDate = tip.targetDrawDate;
    if (targetDate != null && !_sameDate(targetDate, draw.drawDate)) {
      return false;
    }

    // Legacy-Tipps ohne Zielziehung duerfen nicht gegen Ziehungen bewertet
    // werden, die vor dem Tipp lagen. Das verhindert rueckwirkende Treffer.
    if (tip.targetDrawType == DrawType.unknown && targetDate == null) {
      final createdDay = DateTime(
        tip.createdAt.year,
        tip.createdAt.month,
        tip.createdAt.day,
      );
      final drawDay = DateTime(
        draw.drawDate.year,
        draw.drawDate.month,
        draw.drawDate.day,
      );
      if (drawDay.isBefore(createdDay)) return false;
    }

    return true;
  }

  String _strategyLabelFromSource(String source) {
    final normalized = source.toLowerCase();
    if (normalized.contains('intervall') || normalized.contains('vew')) {
      return 'Intervall-Modell';
    }
    if (normalized.contains('voll')) return 'Vollsystem';
    if (normalized.contains('system')) return 'Systemschein';
    if (normalized.contains('random')) return 'Zufallstipp';
    if (normalized.contains('analysis')) return 'Analyse-Tipp';
    return source.trim().isEmpty ? 'Tipp' : source;
  }
  ...
  Future<List<TipEvaluationResult>> evaluateSavedTipsAgainstSelectedDraw() async {
    ...
    for (var i = 0; i < eligibleTips.length; i++) {
      final tip = eligibleTips[i];
